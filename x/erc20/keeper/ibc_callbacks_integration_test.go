...
import (
	...
	"github.com/elysiumstation/blackfury/v13/contracts"
	ibctesting "github.com/elysiumstation/blackfury/v13/ibc/testing"
	"github.com/elysiumstation/blackfury/v13/testutil"
	teststypes "github.com/elysiumstation/blackfury/v13/types/tests"
	"github.com/elysiumstation/blackfury/v13/utils"
	claimstypes "github.com/elysiumstation/blackfury/v13/x/claims/types"
	"github.com/elysiumstation/blackfury/v13/x/erc20/types"
...
var _ = Describe("Convert receiving IBC to Erc20", Ordered, func() {
	...
	Describe("enabled params and registered uosmo", func() {
		...
		It("should transfer and not convert afury", func() {
			...
			afuryInitialBalance := s.app.BankKeeper.GetBalance(s.BlackfuryChain.GetContext(), receiverAcc, utils.BaseDenom)

			// 1. Send afury from Blackfury to Osmosis
			s.SendAndReceiveMessage(s.pathOsmosisBlackfury, s.BlackfuryChain, utils.BaseDenom, amount, receiver, sender, 1, "")

			afuryAfterBalance := s.app.BankKeeper.GetBalance(s.BlackfuryChain.GetContext(), receiverAcc, utils.BaseDenom)
			...
			// 2. Send afury IBC coins from Osmosis to Blackfury
			ibcCoinMeta := fmt.Sprintf("%s/%s", teststypes.AfuryDenomtrace.Path, teststypes.AfuryDenomtrace.BaseDenom)
			s.SendBackCoins(s.pathOsmosisBlackfury, s.IBCOsmosisChain, teststypes.AfuryIbcdenom, amount, sender, receiver, 1, ibcCoinMeta)
			...
			afuryFinalBalance := s.app.BankKeeper.GetBalance(s.BlackfuryChain.GetContext(), receiverAcc, utils.BaseDenom)
			...
			ibcCoinsBalance := s.app.BankKeeper.GetBalance(s.BlackfuryChain.GetContext(), receiverAcc, teststypes.AfuryIbcdenom)
	...
	Describe("Performing claims with registered coin", func() {
		...
		It("it should perform the claim and convert the received tokens", func() {
			...
			receiverInitialAfuryBalance := s.app.BankKeeper.GetBalance(s.BlackfuryChain.GetContext(), receiverAcc, utils.BaseDenom)
			...
			receiverFinalAfuryBalance := s.app.BankKeeper.GetBalance(s.BlackfuryChain.GetContext(), receiverAcc, utils.BaseDenom)
